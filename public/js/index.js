...
if (window.matchMedia("(min-width:768px)").matches) {
	...
	$(window).scroll(function() {
		...
		const homeEl = $(".menu-link[href='#home']")[1];
		const presentationEl = $(".menu-link[href='#presentation']")[1];
		const presentationPosition = Math.round($("#presentation").position().top);
		const serviceEl = $(".menu-link[href='#services']")[1];
		const servicesPosition = Math.round($("#services").position().top);
		const partnersEl = $(".menu-link[href='#partners']")[1];
		const partnersPosition = Math.round($("#partners").position().top);
		const contactEl = $(".menu-link[href='#contact']")[1];
...
$(window).resize(function() {
	...
	if (window.matchMedia("(min-width:768px)").matches) {
		...
		$(window).scroll(function() {
			...
			const homeEl = $(".menu-link[href='#home']")[1];
			const presentationEl = $(".menu-link[href='#presentation']")[1];
			const presentationPosition = Math.round(
				$("#presentation").position().top,
			);
			const serviceEl = $(".menu-link[href='#services']")[1];
			const servicesPosition = Math.round($("#services").position().top);
			const partnersEl = $(".menu-link[href='#partners']")[1];
